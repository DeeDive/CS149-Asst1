...
void clampedExpVector(float *values, int *exponents, float *output, int N)
{
  ...
  __cs149_vec_float clamp_threshold = _cs149_vset_float(9.999999f);

  __cs149_mask mask;
  __cs149_mask maskAll = _cs149_init_ones();          // all in use
  __cs149_mask maskAllNot = _cs149_mask_not(maskAll); // all not in use
  for (int i = 0; i < N; i += VECTOR_WIDTH)
  {
    if (i + VECTOR_WIDTH - 1 < N)
      mask = _cs149_init_ones(VECTOR_WIDTH);
    else
      mask = _cs149_init_ones(N - i);
    __cs149_mask maskExponentIsZero{maskAll}; // suppose all exponents are not zero

    _cs149_vload_float(x, values + i, mask);
    _cs149_vload_int(y, exponents + i, mask);
    _cs149_veq_int(maskExponentIsZero, y, zero, mask); // exponents that are really not zero are now masked; we invert the mask.

    __cs149_mask maskExponentIsNotZero = _cs149_mask_not(maskExponentIsZero);
    maskExponentIsZero = _cs149_mask_and(mask, maskExponentIsZero);
    maskExponentIsNotZero = _cs149_mask_and(mask, maskExponentIsNotZero);

    _cs149_vset_float(result, 1.f, maskExponentIsZero); // x^0 = 1.f; some values at the masked position are meaningless

    _cs149_vmove_float(result, x, maskExponentIsNotZero); // else y > 0
    _cs149_vsub_int(y, y, one, maskExponentIsNotZero);
    _cs149_veq_int(maskExponentIsZero, y, zero, maskExponentIsNotZero); // exponents that are really not zero are now masked; we invert the mask.
    maskExponentIsNotZero = _cs149_mask_not(maskExponentIsZero);
    maskExponentIsZero = _cs149_mask_and(mask, maskExponentIsZero);
    maskExponentIsNotZero = _cs149_mask_and(mask, maskExponentIsNotZero);

    while (_cs149_cntbits(maskExponentIsNotZero) != 0)
    {
      _cs149_vmult_float(result, result, x, maskExponentIsNotZero);
    ...
    _cs149_vgt_float(clamp_mask, result, clamp_threshold, mask);
    _cs149_vmove_float(result, clamp_threshold, clamp_mask);
